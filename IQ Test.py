...
given numbers differs from the others. Bob observed that one number usually differs from the others in evenness.
...
def iq_test(numbers):
    x = numbers.split()
    odd = [i for i in x if int(i) % 2 == 0]
    even = [i for i in x if int(i) % 2 != 0]
    if len(odd) > len(even):
        return x.index(even[0]) + 1
    else:
        return x.index(odd[0]) + 1
